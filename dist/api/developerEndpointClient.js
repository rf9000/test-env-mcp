...
export class DeveloperEndpointClient {
    ...
    async publishApp(params) {
        ...
        while (attempt < maxAttempts) {
            ...
            try {
                ...
                const url = this.buildDeveloperEndpointUrl(params.environmentUrl, params.schemaUpdateMode ?? 'synchronize');
                ...
                if (axios.isAxiosError(error)) {
                    ...
                    if (status === 401 || status === 403) {
                        throw new AuthError('Publishing failed: Invalid credentials after retry. Verify environment permissions and user access.', {
                    ...
                    // Schema conflict
                    if (status === 409) {
                        throw new ConflictError('Publishing failed: Schema conflict detected. ' +
                    ...
                    // Other HTTP errors
                    const errorMessage = ErrorService.redact(error.response?.data?.message ||
                        error.response?.data?.error ||
                        error.message ||
                        'Unknown error');
                    throw new Error(`Publishing failed with HTTP ${status}: ${errorMessage}`);
                }
                // Non-Axios errors
                throw error;
            }
    ...
    createHttpsAgent(environmentUrl) {
        ...
        // Check configuration
        const allowInsecure = this.credentialsService['configService'].get('auth.allowInsecureCertificates', false) || false;
        return new https.Agent({
            rejectUnauthorized: !(isLocalhost || allowInsecure)
        });
    }
}
//# sourceMappingURL=developerEndpointClient.js.map