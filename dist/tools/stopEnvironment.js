...
export const StopEnvironmentInputSchema = z.object({
    environmentId: z
        ...
        .describe('The unique identifier of the environment to stop. Use list_environments to find valid IDs.'),
    wait: z
        .enum(['none', 'untilStopped'])
        .optional()
        .default('none')
        .describe('Wait behavior: "none" (return immediately after issuing command) or "untilStopped" (wait until environment is fully stopped). Default: "none"')
});
/**
 ...
 */
export async function executeStopEnvironment(environmentService, input) {
    try {
        // Validate input
        const validated = StopEnvironmentInputSchema.parse(input);
        // Execute service method
        const result = await environmentService.stopEnvironment(validated.environmentId, { wait: validated.wait });
        return result;
    }
    catch (error) {
...
//# sourceMappingURL=stopEnvironment.js.map