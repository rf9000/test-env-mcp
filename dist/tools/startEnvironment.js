...
export const StartEnvironmentInputSchema = z.object({
    environmentId: z
        ...
        .describe('The unique identifier of the environment to start. Use list_environments to find valid IDs.'),
    wait: z
        .enum(['none', 'untilRunning'])
        .optional()
        .default('none')
        .describe('Wait behavior: "none" (return immediately after issuing command) or "untilRunning" (wait until environment is fully running). Default: "none"')
});
/**
 ...
 */
export async function executeStartEnvironment(environmentService, input) {
    try {
        // Validate input
        const validated = StartEnvironmentInputSchema.parse(input);
        // Execute service method
        const result = await environmentService.startEnvironment(validated.environmentId, { wait: validated.wait });
        return result;
    }
    catch (error) {
...
//# sourceMappingURL=startEnvironment.js.map